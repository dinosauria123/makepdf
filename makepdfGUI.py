...
class Application(tk.Frame):
    ...
    def makepdf(self):
        ...
        if glob.glob(dir+"/hocr-pdf.exe") == []:
            messagebox.showerror("Error", 'No hocr-pdf.exe. \nCheck hocr-pdf.exe and this file \nare in the same directroy.') 
            sys.exit(1)
        ...
        for name in files:
            ...
            gcv2hocr = os.path.dirname(__file__)+"/gcv2hocr.exe"
            command = [gcv2hocr, json, hocr]
            subprocess.call(command)

        print("Generating out.pdf")
        var.set("Generating out.pdf")
        hocr_pdf = os.path.dirname(__file__)+"/hocr-pdf.exe"
        out0 = dname+"/out0.pdf"

        command = [hocr_pdf, "--savefile", out0, dname]
        subprocess.call(command)
